...
//        _sessionConfig.session = self.session;
        if ((self.messageExtDictionary != nil)
            && ([[self.messageExtDictionary valueForKey:MESSAGE_FROM_VIEW] isEqualToNumber:MESSAGE_FROM_VIEW_SEARCH])) {
            _sessionConfig = [[SAMCSearchSessionConfig alloc] initWithSession:self.session];
            _sessionConfig.session = self.session;
        }else{
            _sessionConfig = [[NTESSessionConfig alloc] init];
            _sessionConfig.session = self.session;
...
//SAMC_BEGIN
#pragma mark - Override deleteMsg
