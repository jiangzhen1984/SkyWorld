...
//SAMC_BEGIN
- (instancetype)initWithSession:(NIMSession *)session messageExtDictionary:(NSDictionary *)extDict
{
...
{
    //SAMC_BEGIN
    if(self.messageExtDictionary){
        if (message.remoteExt == nil) {
            message.remoteExt = self.messageExtDictionary;
        }else{
            NSMutableDictionary *extDic = [[NSMutableDictionary alloc] initWithDictionary:message.remoteExt];
            [extDic addEntriesFromDictionary:self.messageExtDictionary];
...
//SAMC_BEGIN
// override by child to process SAMCQuestionMessage
//SAMC_END
