...
@interface SAMCSessionListViewController ()<NIMConversationManagerDelegate,NIMUserManagerDelegate>
...
- (void)viewDidLoad {
    ...
    _recentSessions = [self filteredSessions:[NIMSDK sharedSDK].conversationManager.allRecentSessions];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (recent.session.sessionType == NIMSessionTypeP2P) {
        ...
        //SAMC_BEGIN
        cell.messagesFromView = self.currentListMessageFromView;
        //SAMC_END
...
- (void)didAddRecentSession:(NIMRecentSession *)recentSession
    ...
    if (![self shouldIncludeRecentSession:recentSession]) {
        return;
    }
...
#pragma mark - Misc
- (NSMutableArray *)filteredSessions:(NSArray *)sessions
{
    NSMutableArray *filterdSessions = [NSMutableArray array];
    for (NIMRecentSession *session in sessions) {
        if ([self shouldIncludeRecentSession:session]) {
            [filterdSessions addObject:session];
        }
    }
    return filterdSessions;
}
