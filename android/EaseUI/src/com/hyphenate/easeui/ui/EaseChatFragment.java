...
public class EaseChatFragment extends EaseBaseFragment {
     ...
     * 传入fragment的参数
     */
    /*SAMC_BEGIN(Chat activity type )*/
    protected String chatActivityType;
    /*SAMC_END(Chat activity type )*/
    ...
    /*SAMC_BEGIN(update group chat name)*/
    ...
    /*SAMC_END(update group chat name)*/
	
    /*SAMC_BEGIN(register broadcast for new question received)*/
	...
	private void registerBroadcastReceiver() {
		...
		intentFilter.addAction(EaseConstant.ACTION_NEW_QUEST_FOR_VENDOR_CHAT_FRAG);

		broadcastReceiver = new BroadcastReceiver() {
			@Override
			public void onReceive(Context context, Intent intent) {
				String from = intent.getExtras().getString(EaseConstant.QUESTION_OWNER);
	...
	/*SAMC_END(register broadcast for new question received)*/
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
	 ...
	 if(chatType == EaseConstant.CHATTYPE_SINGLE){
	 	chatActivityType = fragmentArgs.getString(EaseConstant.CHAT_ACTIVITY_TYPE);
	 }else{
	...
	/*SAMC_BEGIN(register broadcast for new question received)*/
	if(chatActivityType!=null && chatActivityType.equals(EaseConstant.CHAT_ACTIVITY_TYPE_VIEW_VENDOR)){
    ...
    protected void onConversationInit(){
	 ...
	 /*SAMC_BEGIN(add conversation attribute)*/
	 if(chatActivityType!=null){
       	 synchronized(conversation){
			if(chatActivityType.equals(EaseConstant.CHAT_ACTIVITY_TYPE_VIEW_SERVICE)){
				String attr = conversation.getExtField();
				if(attr == null){
		  			conversation.setExtField(EaseConstant.CONVERSATION_ATTR_VIEW_SERVICE);
				}else if(!attr.contains(EaseConstant.CONVERSATION_ATTR_VIEW_SERVICE)){
					conversation.setExtField(attr+EaseConstant.CONVERSATION_ATTR_VIEW_SERVICE);
				}
			}else if(chatActivityType.equals(EaseConstant.CHAT_ACTIVITY_TYPE_VIEW_CHAT)){
				String attr = conversation.getExtField();
				if(attr == null){
		  			conversation.setExtField(EaseConstant.CONVERSATION_ATTR_VIEW_CHAT);
				}else if(!attr.contains(EaseConstant.CONVERSATION_ATTR_VIEW_CHAT)){
					conversation.setExtField(attr+EaseConstant.CONVERSATION_ATTR_VIEW_CHAT);
				}
			}else if(chatActivityType.equals(EaseConstant.CHAT_ACTIVITY_TYPE_VIEW_VENDOR)){
				String attr = conversation.getExtField();
				if(attr == null){
		  			conversation.setExtField(EaseConstant.CONVERSATION_ATTR_VIEW_VENDOR);
				}else if(!attr.contains(EaseConstant.CONVERSATION_ATTR_VIEW_VENDOR)){
					conversation.setExtField(attr+EaseConstant.CONVERSATION_ATTR_VIEW_VENDOR);
				}
			}			
		}    
	}  	 
	 /*SAMC_END(add conversation attribute)*/
        // 把此会话的未读数置为0
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) { 
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                Log.e("test","onActivityResult selectPicFromLocal");
                if (data != null) {
    ...
    protected void sendMessage(EMMessage message){
	 ...
	 /*SAMC_BEGIN(set msg attribute when send)*/
	 if(chatActivityType!=null){
		message.setAttribute(EaseConstant.CHAT_ACTIVITY_TYPE,chatActivityType);
		if(chatActivityType.equals(EaseConstant.CHAT_ACTIVITY_TYPE_VIEW_VENDOR)){
			String to = message.getTo()==null?message.getUserName():message.getTo();
			List<String> quest_list = clientDBInterface.getNotResponsedQuestion(to);
			if(quest_list!=null){
				String questions = null;
				for(String quest:quest_list){
					questions=quest+" ";
				}
				if(questions!=null){
					message.setAttribute(EaseConstant.CHAT_QUESTIONS_BACK,questions);
				}
			}
		}
    ...
    /*SAMC_BEGIN(add db opt callback in EaseUI lib)*/
    protected ClientDBInterface clientDBInterface;
    public void setClientDBCallBack(ClientDBInterface clientDBInterface){
        this.clientDBInterface = clientDBInterface;
    }
    public interface ClientDBInterface{
         List<String> getNotResponsedQuestion(String sender);
    }
    /*SAMC_END(add db opt callback in EaseUI lib)*/
