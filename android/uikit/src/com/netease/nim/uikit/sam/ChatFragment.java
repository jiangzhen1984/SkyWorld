 ...
 */
public class ChatFragment extends TFragment implements TAdapterDelegate {
	...
	private void registerBroadcastReceiver() {
		...
		intentFilter.addAction(NimConstants.ACTION_CHAT_UPDATE);
            ...
            public void run() {
                ...
                NIMClient.getService(MsgService.class).queryRecentContacts().setCallback(new RequestCallbackWrapper<List<RecentContact>>() {
                    ...
                    public void onResult(int code, List<RecentContact> recents, Throwable exception) {
                        ...
                        List<RecentContact> chat_recents = new ArrayList<RecentContact>();
			    for(RecentContact result:recents){
				 if(result.getSessionType()!=SessionTypeEnum.P2P){
                                chat_recents.add(result);
				 }else if((result.getTag() & NimConstants.RECENT_TAG_SHOW_IN_CHAT)!=0){
                                chat_recents.add(result);
				 }
			    }

			    loadedRecents = chat_recents;
        ...
        public void onEvent(List<RecentContact> messages) {
            ...
            for (RecentContact msg : messages) {
                ...
                if (index >= 0) {
                    items.remove(index);
			items.add(msg);
                }else if(msg.getSessionType() != SessionTypeEnum.P2P){
                    items.add(msg);
                }
