...
public class DBManager
{
	...
	public SendQuestionWraper querySendQuestionWraper(String question_id){
		String table = DatabaseHelper.TABLE_NAME_SEND_QUESTION;
		SendQuestionWraper question = null;
		Cursor c = db.query(table,null,"question_id=?",new String[]{question_id},null,null,null);

		if(c.getCount()>1){
			SamLog.e(TAG, "Fatal Error for query send question wraper");
			throw new RuntimeException("Fatal Error for query send question wraper!");
		}
		
		while(c.moveToNext()){
			question = new SendQuestionWraper();
			question.id = c.getLong(c.getColumnIndex("id"));
			question.question_id = c.getString(c.getColumnIndex("question_id"));
 			question.question = c.getString(c.getColumnIndex("question"));
			question.status = c.getInt(c.getColumnIndex("status"));
	...
	public List<SendQuestionWraper> querySendQuestionByUsernameWraper(String username){
		String table = DatabaseHelper.TABLE_NAME_SEND_QUESTION;
		List<SendQuestionWraper> SendQuestionArray = new ArrayList<SendQuestionWraper>();
		SendQuestionWraper question = null;
		Cursor c = db.query(table,null,"senderusername=?",new String[]{username},null,null,"sendtime desc");
		...
		while(c.moveToNext()){
			...
			question.sendercellphone = c.getString(c.getColumnIndex("sendercellphone"));
			question.senderusername = c.getString(c.getColumnIndex("senderusername"));
			SendQuestionArray.add(question);
		}

		c.close();

		return SendQuestionArray;
	}

	public long addRespQuestion(RespQuest record)
	{
		//id|receiver_name|sender_name|question_id
		String table = DatabaseHelper.TABLE_NAME_RESP_QUEST;
		
		ContentValues cv = new ContentValues();
		cv.put("receiver_name",record.receiver_name);
		cv.put("sender_name",record.sender_name);
		cv.put("question_id",record.question_id);

		return db.insert(table,null,cv);		
	}

	public RespQuest queryRespQuest(String receiver_name,String sender_name,String question_id){
		String table = DatabaseHelper.TABLE_NAME_RESP_QUEST;
		RespQuest rsp_quest = null;
		Cursor c = db.query(table,null,"receiver_name=? and sender_name=? and question_id=?",new String[]{receiver_name,sender_name,question_id},null,null,null);

		if(c.getCount()>1){
			SamLog.e(TAG, "Fatal Error for query resp question");
			throw new RuntimeException("Fatal Error for query resp question!");
		}
		
		while(c.moveToNext()){
			rsp_quest = new RespQuest();
			rsp_quest.id = c.getLong(c.getColumnIndex("id"));
			rsp_quest.receiver_name = c.getString(c.getColumnIndex("receiver_name"));
			rsp_quest.sender_name = c.getString(c.getColumnIndex("sender_name"));
			rsp_quest.question_id = c.getString(c.getColumnIndex("question_id"));
		}
	...
	public List<RespQuest> queryRespQuest(String receiver_name,String sender_name){
		String table = DatabaseHelper.TABLE_NAME_RESP_QUEST;
		RespQuest rsp_quest = null;
		List<RespQuest> rsp_quest_list = new ArrayList<RespQuest>();
		Cursor c = db.query(table,null,"receiver_name=? and sender_name=?",new String[]{receiver_name,sender_name},null,null,null);
		...
		while(c.moveToNext()){
			...
			rsp_quest.sender_name = c.getString(c.getColumnIndex("sender_name"));
			rsp_quest.question_id = c.getString(c.getColumnIndex("question_id"));
			rsp_quest_list.add(rsp_quest);
		}

		c.close();

		return rsp_quest_list;
	}
