...
public class HttpCommClient {
	...
	public boolean signin(String country_code,String username,String password){
		if(username==null || password==null){
			...
			ret = SignService.RET_SI_FROM_SERVER_UP_ERROR;
			return false;
		}
		...
		try{
			...
			if(statusCode == HttpStatus.SC_OK ){
				...
				ret = SignService.RET_SI_FROM_CLIENT_CONNECT_FAILED;
				return false;
			}
		
		}catch (JSONException e) {  
			throw new RuntimeException("jsone exception in signin");
		} catch (ClientProtocolException e) {
			SamLog.e(TAG,"ClientProtocolException");
			e.printStackTrace(); 
			ret = SignService.RET_SI_FROM_CLIENT_CONNECT_FAILED;
...
public boolean uploadavatar(String filePath, String token){
		...
		try {
			...
			if(conn.getResponseCode() != HttpURLConnection.HTTP_OK){
				SamLog.e(TAG,"upload avatar http status:"+conn.getResponseCode());
				return false; 
			}
