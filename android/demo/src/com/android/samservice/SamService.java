   ...
   public static final int MSG_UPGRADE_TO_SERVICER=4;
	...
	public static synchronized SamService getInstance(Activity activity){
		if(mSamService == null){
			mContext = DemoCache.getContext();
			if(mContext == null){
				SamLog.e("test","mContext is null");
	...
	public void onActivityLaunched(Handler push_handler){
		servicer_question_hndl = new WeakReference <Handler>(push_handler);
	}
    ...
    private final class SamPushServiceHandler extends Handler{
		...
		if(needPushToUI(msg.what) ){
			if(servicer_question_hndl == null){
			...
			if(sq_hndl == null){
				SamLog.e(TAG, "sq_hndl has been destroy, drop cbMsg ...");
				return;
			}
		}else{
			sq_hndl = null;
			...
			case MSG_PUSH_MSG_GOTQUESTION:
				...
				if(current_user.usertype == LoginUser.USER) {
					...
					if(phinfoq.opt == 0){
						...
						msgid = SamProsFragment.MSG_QUESTION_RECEIVED;
	...
	private void do_send_question(SamCoreObj samobj){
		...
		if(hcc.sendquestion(cbobj.qinfo.question,token)){
			...
			if(hcc.ret == RET_SEND_QUESTION_FROM_SERVER_OK){
				...
				SendQuestion question = new SendQuestion(hcc.question_id,cbobj.qinfo.question);
