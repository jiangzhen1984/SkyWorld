...
import com.android.samchat.IndicatorFragmentActivity.TabInfo;
...
public class TitleIndicator extends LinearLayout implements View.OnClickListener,
	...
	private int getIcon_selected(int pos) {
		int ret = 0;
		if (mTabs != null && mTabs.size() > pos) {
			ret = mTabs.get(pos).getIcon_selected();
		}
		return ret;
	}
	...
	public void init(int startPos, List<TabInfo> tabs, ViewPager mViewPager) {
		...
		for (int i = 0; i < mTotal; i++) {
			add(i, getTitle(i), getIcon(i), getIcon_selected(i));
		}
	...
	protected void add(int index, String label, int icon, int icon_selected) {
		...
		final ImageView iv = (ImageView) tabIndicator.findViewById(R.id.tab_title_icon);
